...
export const ui = {
  es: {
    ...
    'exercise1.description': 'Este ejercicio se basa en la forma despejada de la ley de Darcy, la cual permite analizar el gradiente hidráulico de manera directa. A continuación se presenta la ecuación fundamental:',
    'exercise1.formula_intro': 'Para este ejercicio se emplea la <span class="lato-bold">fórmula despejada</span> del gradiente hidráulico:',
    'exercise1.observation': 'En esta sección se puede observar cómo el gradiente hidráulico (<em>i</em>) varía en función de la conductividad (<em>K</em>), el caudal (<em>Q</em>) y el área (<em>A</em>).',
    'exercise1.how_it_works': '⚙️ Cómo funciona',
    'exercise1.instructions': 'Ajusta los controles para observar cómo cada parámetro influye en la visualización del gradiente hidráulico:',
    'exercise1.k_desc': '<span class="lato-bold">K (Conductividad)</span>: regula la facilidad con la que el agua fluye a través del medio.',
    'exercise1.a_desc': '<span class="lato-bold">A (Área)</span>: determina la cantidad de flujo visible en los tubos.',
    'exercise1.q_desc': '<span class="lato-bold">Q (Caudal)</span>: establece el volumen total de flujo en el sistema.',
    'exercise1.slider_help': 'Utiliza los sliders o introduce los valores manualmente para explorar cómo las variaciones de cada parámetro afectan el gradiente hidráulico.',
    'exercise1.manual_data': 'Datos manuales',
    'exercise1.manual_warning': '(La representación visual puede distorsionarse)',
    ...
    'exercise2.info.para1': 'El aparato consiste en un cilindro de área transversal <b>A</b> que está lleno de un medio poroso, como arena. El agua se introduce lentamente en el recipiente de la izquierda y fluye a través del cilindro lleno de arena hasta que los poros se saturan completamente.',
    ...
    'exercise2.info.para3': 'En ese momento se alcanza una condición de <b>flujo en estado estacionario</b>, donde la cantidad de agua que entra es igual a la que sale. El valor de <b>Q</b> representa el caudal volumétrico del agua a través del cilindro (es decir, volumen por unidad de tiempo, como m³/s, L/s o gal/min).',
    'exercise2.info.para4': 'En este experimento, el flujo de agua se mueve desde una zona de mayor energía hidráulica hacia una de menor energía. Esta diferencia de energía se denomina <b>gradiente hidráulico</b>, y puede expresarse mediante la relación:',
    'exercise2.info.para5': 'donde <b>Δh</b> es la diferencia de carga hidráulica entre los dos puntos, y <b>ΔL</b> es la distancia entre los piezómetros. El flujo es paralelo al eje del cilindro y depende de las propiedades del medio (porosidad y conductividad hidráulica).',
    'exercise2.info.para6': 'En condiciones naturales, el flujo subterráneo se desarrolla dentro de un medio tridimensional mucho más complejo. Los gradientes hidráulicos y las direcciones de flujo pueden variar tanto en magnitud como en dirección dependiendo de la estructura del subsuelo, la presencia de capas confinantes y la heterogeneidad del terreno.',
    'exercise2.info.para7': 'Si instalamos pozos para medir el gradiente hidráulico sin conocer la orientación del estrato o la inclinación del medio, sólo podemos deducir que hay un componente de flujo horizontal. Sin embargo, el flujo también podría tener una componente vertical (ascendente o descendente) dependiendo de la inclinación del medio o de la disposición de los límites confinantes.',
    'exercise2.info.para8': 'En algunos casos, la inclinación puede ser tan pronunciada que el flujo se acerca a una dirección vertical. Este análisis subraya la importancia de conocer la <b>orientación de los límites confinantes</b> y otras características geológicas para interpretar correctamente los datos de carga hidráulica y determinar la dirección real del flujo.',
    ...
    'exercise3.help_line2': 'Añadir divisiones de arena incrementa la carga izquierda (h₁) en 1 ft por división.',
    'exercise3.help_line3': 'Las divisiones aparecen solo dentro de la zona de arena.',
    'exercise3.help_line4': 'El nivel de agua se puede ajustar entre 0 y 12 ft usando cualquier tipo de entrada.',
    'exercise4.title': 'Diagrama Interactivo de Acuíferos',
    'exercise4.intro': 'Ajusta los parámetros (o usa modo Manual). El diagrama reacciona en tiempo real; a la derecha verás fórmulas y resultados.',
  ...
  en: {
    ...
    'exercise1.description': 'This exercise is based on the rearranged form of Darcy\'s Law, which allows direct analysis of the hydraulic gradient. The fundamental equation is presented below:',
    'exercise1.formula_intro': 'This exercise uses the <span class="lato-bold">rearranged formula</span> for hydraulic gradient:',
    'exercise1.observation': 'In this section you can observe how the hydraulic gradient (<em>i</em>) varies as a function of conductivity (<em>K</em>), flow rate (<em>Q</em>), and area (<em>A</em>).',
    'exercise1.how_it_works': '⚙️ How It Works',
    'exercise1.instructions': 'Adjust the controls to see how each parameter affects the hydraulic gradient visualization:',
    'exercise1.k_desc': '<span class="lato-bold">K (Conductivity)</span>: regulates how easily water flows through the medium.',
    'exercise1.a_desc': '<span class="lato-bold">A (Area)</span>: determines the amount of visible flow in the tubes.',
    'exercise1.q_desc': '<span class="lato-bold">Q (Flow rate)</span>: sets the total flow volume in the system.',
    'exercise1.slider_help': 'Use the sliders or enter values manually to explore how variations in each parameter affect the hydraulic gradient.',
    ...
    'exercise2.info.para1': 'The apparatus consists of a cylinder of cross-sectional area <b>A</b> that is filled with a porous medium, such as sand. Water is slowly introduced into the left container and flows through the sand-filled cylinder until the pores are completely saturated.',
    ...
    'exercise2.info.para3': 'At that moment, a <b>steady-state flow</b> condition is reached, where the amount of water entering equals the amount leaving. The value of <b>Q</b> represents the volumetric flow rate of water through the cylinder (i.e., volume per unit time, such as m³/s, L/s, or gal/min).',
    'exercise2.info.para4': 'In this experiment, water flows from an area of higher hydraulic energy toward one of lower energy. This energy difference is called the <b>hydraulic gradient</b>, and can be expressed by the relationship:',
    'exercise2.info.para5': 'where <b>Δh</b> is the hydraulic head difference between the two points, and <b>ΔL</b> is the distance between the piezometers. Flow is parallel to the cylinder axis and depends on the properties of the medium (porosity and hydraulic conductivity).',
    'exercise2.info.para6': 'Under natural conditions, groundwater flow develops within a much more complex three-dimensional medium. Hydraulic gradients and flow directions can vary in both magnitude and direction depending on subsurface structure, the presence of confining layers, and terrain heterogeneity.',
    'exercise2.info.para7': 'If we install wells to measure the hydraulic gradient without knowing the orientation of the stratum or the inclination of the medium, we can only deduce that there is a horizontal flow component. However, the flow could also have a vertical component (upward or downward) depending on the inclination of the medium or the arrangement of confining boundaries.',
    'exercise2.info.para8': 'In some cases, the inclination may be so pronounced that flow approaches a vertical direction. This analysis underscores the importance of knowing the <b>orientation of confining boundaries</b> and other geological features to correctly interpret hydraulic head data and determine the actual flow direction.',
    ...
    'exercise3.help_line2': 'Adding sand divisions increases the left head (h₁) by 1 ft per division.',
    'exercise3.help_line3': 'Divisions appear only inside the sand zone.',
    'exercise3.help_line4': 'Water level can be adjusted between 0 and 12 ft using either input type.',
    'exercise4.title': 'Interactive Aquifer Diagram',
    'exercise4.intro': 'Adjust the parameters (or use Manual mode). The diagram reacts in real time; on the right you will see formulas and results.',
